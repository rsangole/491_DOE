...
delay
temp <- soup$Temp 
interaction.plot(delay, temp, soup$y, type="b",pch=c(24,18,22), 
          leg.bty="o", main="Interaction Plot for Mixing Temperature by Delay time", 
...
interaction.plot(delay, BatchWt, soup$y, type="b",pch=c(18,24,22), 
          leg.bty="o", main="Interaction Plot for Batch Weight by Delay time", 
          xlab="Delay Time (days)", ylab="Average S.D. Fill Weight") 
# Example 5 p. 206 
library(FrF2) 
...
# Example 7 p. 211
library(FrF2) 
culture <- FrF2( 16, generators = c("BCD", "ACD", "ABC", "ABD"), 
                 randomize = FALSE) 
...
culture2 <- FrF2( 16, 5, factor.names = c("B", "C", "E", "G", "H"), 
                  randomize = FALSE) 
y <- c(3.37, 3.55, 3.78, 2.81, 5.53, 10.43, 5.35, 11.57, 
       2.93, 7.23, 3.9, 10.83, 11.69, 10.59, 4.92, 7.39) 
...
interaction.plot(FermTime, RiceBran, culture2$y, type = "b", 
          pch=c(18,24,22), leg.bty="o", main="Interaction Plot for Fermentation Time and Level of Rice Bran", 
          xlab = "Fermentation Time(hrs)", ylab = "Biomass") 
# Figure 6.8 p. 216 
Urea <- 1*((as.numeric(culture2$C)-1.5 ) / .5) + 1 
AmonSulf <- 1*((as.numeric(culture2$E)-1.5) / .5) + 1
interaction.plot(AmonSulf, Urea, culture2$y, type = "b", 
                 pch=c(18,24,22), leg.bty="o", main="Interaction Plot of Urea and Ammonium Sulfate", 
                 xlab = "Ammonium Sulfate(g/L)", ylab = "Biomass") 
# Figure 6.9 p. 216 
Urea <- 1*((as.numeric(culture2$C)-1.5 ) / .5) + 1 
FermTime <- 12*((as.numeric(culture2$H)-1.5) / .5) + 84 
interaction.plot(FermTime, Urea, culture2$y, type = "b", pch=c(18,24,22), 
               leg.bty="o", main="Interaction Plot of Fermentation Time and Urea", 
               xlab = "Fermentation Time(hrs)", ylab = "Biomass")
...
interaction.plot(AmonSulf, SodPhos, culture3$y, type = "b", 
                 pch=c(18,24,22), leg.bty="o", main="Rice Bran = 10g/L", 
                 xlab = "Ammonium Sulfate(g/L)", ylab = "Biomass") 
...
interaction.plot(AmonSulf, SodPhos, culture4$y, type = "b", 
                 pch=c(18,24,22), leg.bty="o", main="Rice Bran = 30g/L", 
                 xlab = "Ammonium Sulfate(g/L)", ylab = "Biomass")
##################################################################
# Example 10 p. 220
library(FrF2) 
des <- FrF2(8, 6, generators = c("AB", "AC", "BC"), randomize = 
              FALSE) 
desa <- fold.design(des, columns = 'full') 
desa 
# Example 11 p. 227
library(FrF2) 
des2 <- FrF2( 8, 7, generators = c("AB", "AC", "BC", "ABC" ), 
              randomize=FALSE) 
...
fr<-1:16 
optim <- optFederov( ~ A + B + F + I(A*D) + I(C*F), 
          data = all, nTrials = 24, criterion = "D", nRepeats = 10, augment = TRUE, rows=fr) 
...
library(leaps)
modpbr<-regsubsets(y ~ (.)^2, data=castfr, method = "exhaustive", 
                   nvmax = 8, nbest = 4) 
rs <- summary(modpbr) 
...
step( null, scope = list(lower = null, upper = up), 
...
des<-oa.design(nlevels=c(3,3,3,3,3,2,2),nruns=36,
               columns="min3",randomize=TRUE,seed=104)
# Example 23 p. 245 
#Create 18 run near-orthogonal array 
library(DoE.base)
cand<-oa.design(nlevels=c(3,3,3,3,3,2,2),nruns=36,columns="min3",seed=104) #Use Federov algorithm to select D-optimal 18-run subset from the from # candidates that will allow estimation of the main effects 
library(AlgDesign)
...
optim<-optFederov(~A+B+C+D+E+F+G+E:F+F:G, cand,nRepeats=10,
                     nTrials=18,criterion="D")
# Example 25 p. 246 
...
#Use Federov algorithm to select D-optimal 12-run subset from the from # candidates that will allow estimation of the main effects 
library(AlgDesign)
...
############################################
library(FrF2) 
design <- FrF2(4, 2, randomize = FALSE)
design
y1 <- c(1.7, 1.36, .97, 0.62)
library(DoE.base) 
design <- add.response( design , y1 )
design
mod1 <- lm(y1 ~ (.)^2, data = design) 
summary(mod1)
library(daewr)
fullnormal(coef(mod1)[-1],alpha=.025)
##(1-.5)/3 = 0.17;(2-0.5)/3=0.5; (3-0.5)/3=0.83;
##Z-score(0.17)=-0.95
##Z-score(0.5)=0
##Z-score(0.83)=0.95
